#-------------------------------------------
# call: bash ./stage_cred.sh <some_instance.lp> <arg>
#-------------------------------------------
...
# echo "Running clingo to compute the ranges"
    ...
    echo "#show in/1." | "$dir"/../../../clingo - "$dir"/heuristic_stage.lp $1 --heuristic=Domain --enum=domRec 0 2>/dev/null | grep 'range' | grep -v 'undec('$2')'| sed 's/)/)./g; s/range//g' > "$dir"/rangeListCred.tmp
