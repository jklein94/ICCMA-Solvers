...
{
    # to be adapted
    echo "MySolver 0.8.16"
    echo "Federico Cerutti <federico.cerutti@acm.org>"
    echo "Thomas Linsbichler <linsbich@dbai.tuwien.ac.at>"
    echo "Francesco Santini <francesco.santini@dmi.unipg.it>"
    echo "Theofrastos Mantadelis <theo.mantadelis@dmi.unipg.it>"
}
...
function solver
{
    ...
    case $task in
        "DC-CO")
            $DIR$SOLVER -q -p -e complete -c $additional $fileinput
        ;;
        "DC-PR")
            $DIR$SOLVER -q -p -e preferred -c $additional $fileinput
        ;;
        "DC-ST")
            $DIR$SOLVER -q -p -e stable -c $additional $fileinput
        ;;
        "DC-SST")
            $DIR$SOLVER -q -p -e semistable -c $additional $fileinput
        ;;
        "DC-STG")
            $DIR$SOLVER -q -p -e stage -c $additional $fileinput
        ;;
        "DC-GR")
            $DIR$SOLVER -q -p -e grounded -c $additional $fileinput
        ;;
        "DC-ID")
            #$DIR$SOLVER -q -p -e ideal -c $additional $fileinput
            #$DIR$SOLVER -p DC-ID -f $fileinput -fo apx -a $additional
            echo "Ciao"
        ;;
        "DS-CO")
            $DIR$SOLVER -q -p -e complete -s $additional $fileinput
        ;;
        "DS-PR")
            $DIR$SOLVER -q -p -e preferred -s $additional $fileinput
        ;;
        "DS-ST")
            $DIR$SOLVER -q -p -e stable -s $additional $fileinput
        ;;
        "DS-SST")
            $DIR$SOLVER -q -p -e semistable -s $additional $fileinput
        ;;
        "DS-STG")
            $DIR$SOLVER -q -p -e stage -s $additional $fileinput
        ;;
        "EE-CO")
            $DIR$SOLVER -q -p -e complete $fileinput
        ;;
        "EE-PR")
            $DIR$SOLVER -q -p -e preferred $fileinput
        ;;
        "EE-ST")
            $DIR$SOLVER -q -p -e stable $fileinput
        ;;
        "EE-SST")
            $DIR$SOLVER -q -p -e semistable $fileinput
        ;;
        "EE-STG")
            $DIR$SOLVER -q -p -e stage $fileinput
        ;;
        "EE-GR")
            $DIR$SOLVER -q -p -e grounded $fileinput
        ;;
        "EE-ID")
            $DIR$SOLVER -q -p -e ideal $fileinput
        ;;
        "SE-CO")
            $DIR$SOLVER -q -p -e complete -n 1 $fileinput
        ;;
        "SE-PR")
            $DIR$SOLVER -q -p -e preferred -n 1 $fileinput
        ;;
        "SE-ST")
            $DIR$SOLVER -q -p -e stable -n 1 $fileinput
        ;;
        "SE-SST")
            $DIR$SOLVER -q -p -e semistable -n 1 $fileinput
        ;;
        "SE-STG")
            $DIR$SOLVER -q -p -e stage -n 1 $fileinput
        ;;
        "SE-GR")
            $DIR$SOLVER -q -p -e grounded -n 1 $fileinput
        ;;
        "SE-ID")
            #$DIR$SOLVER -q -p -e ideal -n 1 $fileinput
            echo "Ciao"
        ;;
        # NEW DYNAMIC PROBLEMS TO BE ADDED HERE
        *)
